...
from catalog.models import Tasks, Programmer, LevelOfDifficulty, StatusOfTask  # Замените 'catalog' на имя вашего приложения
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Создание 200 задач
        for _ in range(200):
            chosen_status = random.choice(statuses)  # Выбор случайного объекта StatusOfTask

            if chosen_status.status == "ready" or chosen_status.status == "somebody is doing this task":
                chosen_programmer = random.choice(programmers) if programmers else None  # Выбор случайного программиста или None
            else:
            ...
            task = Tasks.objects.create(
                ...
                description=fake.paragraph(nb_sentences=3),  # Генерация случайного описания
                level=chosen_level,
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Успешно создана задача: {task.name}\n"
            ))
